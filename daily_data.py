...
def update_and_store_data():
    ...
    if total_expenditure:
        ...
        for item_id, entry_widget in entry_sold.items():
            ...
            table_treeview.item(item_id, values=(item_id, table_treeview.item(item_id, 'values')[1], items_sold, cost, total_cost))

        total_items_sold = sum(int(table_treeview.item(item_id, 'values')[2]) for item_id in entry_sold)
        total_cost = sum(float(table_treeview.item(item_id, 'values')[4]) for item_id in entry_sold)
